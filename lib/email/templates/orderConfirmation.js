// Responsive, inbox-safe order confirmation template with DBT color system (inlined for email support)
...
export function buildOrderConfirmation({ order, items = [], downloads = [], brand = {} }) {
  // Pull brand/app from env unless explicitly provided
  const {
...
<body>
  ...
  const text = [
    ...
    (appUrl && orderId) ? `View order: ${appUrl}/orders/${orderId}` : "",
  ].filter(Boolean).join("\n");
